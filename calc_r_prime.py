...
def calc_r_prime(d):
    ...
    if config.plot_setup:
        ...
        fig, ax = plt.subplots(figsize=(config.COLUMNS*config.ACTIVE_ARRAYS/2, config.ROWS/1.8))
        ...
        dx = config.ELEMENT_DISTANCE*0.1        # displacement of mic index text
        dy = config.ELEMENT_DISTANCE*0.1        # displacement of mic index text
        FS = 11     # font size of plot texts
        S = 100     # size of scatter point
        element = 0
        for array in range(config.ACTIVE_ARRAYS):
            ...
            for mic in range(len(mics_array)): #range(r_prime.shape[1]): #range(int(len(active_mics)/config.active_arrays)):
                ...
                plt.text(x-dx, y+dy, str(active_mics[element]), fontsize = FS)
                element += 1
        ...
        # save figure
        filename = 'array_setup_A'+ str(config.ACTIVE_ARRAYS)
    ...
    if config.plot_setup:
        ...
        color_arr = ['r', 'b', 'g','m'] # colors of elements in the different arrays
        dx = config.ELEMENT_DISTANCE*0.1 * 10**2    # displacement of mic index text
        dy = config.ELEMENT_DISTANCE*0.2 * 10**2    # displacement of mic index text
...
calc_r_prime(config.ELEMENT_DISTANCE)
